...
public class IndexTester {
	...
	private static final String format = "Your output file %s does not match the expected output.";
	...
	public void testIndexSimple() {
		...
		String[] args = new String[] { "-d", dir.toAbsolutePath().toString(),
				"-i", name };
	...
	/**
	 * Tests if the correct index is generated for the simple test case when no
	 * output file name is provided. (The default should be invertedindex.txt.)
	 */
	@Test
	...
	/**
	 * Tests if the correct index is generated for the simple test case when the
	...
	public void testIndexReversed() {
		...
		String[] args = new String[] { "-i", "invertedindex-reversed.txt",
				"-d", dir.toAbsolutePath().toString() };

		Driver.main(args);

		Assert.assertTrue(String.format(format, name), BaseTester.testFiles(
				Paths.get("invertedindex-reversed.txt"), out));
	}
