...
public class HTMLLinkTester {
	...
	public void multipleLinks() {
		...
		String[] expected = new String[] { "http://www.cs.usfca.edu/",
				"http://www.usfca.edu", "http://cs.usfca.edu/grad.html" };

		testText(test, expected);
	}

	/**
	 * Tests whether running the {@link HTMLLinkParser} on the provided text
	 ...
	 *            links
	 * @param expected
	 *            - array of expected links (order matters)
	 */
	private static void testText(String text, String[] expected) {
		ArrayList<String> results = HTMLLinkParser.listLinks(text);

		String debug = String.format("Text: %s, Expected: %s, Got: %s", text,
				Arrays.toString(expected), results);

		Assert.assertArrayEquals(debug, expected,
